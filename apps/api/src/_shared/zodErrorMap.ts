...
export const friendlyErrorMap: ZodErrorMap = (issue, ctx) => {
  ...
  switch (issue.code) {
    case 'invalid_type':
      if (issue.received === 'undefined' || issue.received === 'null') return { message: `${f} is required` };
      if (issue.expected === 'number')  return { message: `${f} must be a number` };
      if (issue.expected === 'string')  return { message: `${f} must be text` };
      if (issue.expected === 'boolean') return { message: `${f} must be yes or no` };
    ...
    case 'too_small':
      ...
      if (issue.type === 'number') {
        if (!issue.inclusive && Number(issue.minimum) === 0) return { message: `${f} must be greater than 0` };
        return { message: issue.inclusive ? `${f} must be at least ${issue.minimum}` : `${f} must be greater than ${issue.minimum}` };
      }
      if (issue.type === 'array') return { message: `Add at least ${issue.minimum} item${Number(issue.minimum) === 1 ? '' : 's'}` };
      return { message: `${f} is too small` };
    case 'too_big':
      if (issue.type === 'string') return { message: `${f} must be at most ${issue.maximum} characters` };
      if (issue.type === 'number') return { message: `${f} must be at most ${issue.maximum}` };
      if (issue.type === 'array')  return { message: `Maximum ${issue.maximum} items allowed` };
      return { message: `${f} is too large` };
    case 'invalid_string':
      if (issue.validation === 'email')    return { message: 'Enter a valid email address' };
      if (issue.validation === 'url')      return { message: `${f} is required — upload the file first` };
      if (issue.validation === 'datetime') return { message: `${f} must be a valid date` };
      if (issue.validation === 'uuid')     return { message: `${f} is invalid — please select again` };
