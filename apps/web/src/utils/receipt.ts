...
export function openSinglePaymentReceipt(d: SinglePaymentReceiptData) {
  ...
  // Customer avatar — same as bill.ts
  ...
  const avatar = d.customerPhotoUrl
    ? `<img src="${d.customerPhotoUrl}" alt="${d.customerName}"
         style="width:52px;height:52px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,255,255,.25);flex-shrink:0"/>`
    : `<div style="width:52px;height:52px;border-radius:50%;background:${avatarBg};
         border:2px solid rgba(255,255,255,.25);flex-shrink:0;display:flex;align-items:center;
         justify-content:center;font-size:19px;font-weight:800;color:#fff">${initials}</div>`;

  const lateStatusHtml = isLate
    ? `<div style="display:inline-flex;align-items:center;gap:4px;background:#fee2e2;border:1px solid #fca5a5;border-radius:20px;padding:3px 10px;font-size:9px;font-weight:700;color:#991b1b">⚠️ ${d.daysLate} din late</div>`
    : `<div style="display:inline-flex;align-items:center;gap:4px;background:#d1fae5;border:1px solid #6ee7b7;border-radius:20px;padding:3px 10px;font-size:9px;font-weight:700;color:#065f46">✓ On time</div>`;
...
.hdr::after{content:'';position:absolute;bottom:-25px;left:30px;width:70px;height:70px;background:rgba(96,165,250,.07);border-radius:50%}
/* ── Customer card ── */
...
/* ── Period card ── */
.period{margin:0 14px 10px;background:linear-gradient(135deg,#eff6ff,#dbeafe);border:1px solid #93c5fd;border-radius:8px;padding:8px 12px;display:flex;align-items:center;justify-content:space-between;gap:8px}
/* ── Footer ── */
...
<div class="card">
  ...
  <div class="hdr">
    <div style="flex:1;min-width:0">
      ...
      <div style="margin-top:4px">
        <span style="display:inline-block;padding:2px 8px;border-radius:20px;font-size:8px;font-weight:700;background:rgba(255,255,255,.12);color:rgba(255,255,255,.75);border:1px solid rgba(255,255,255,.15)">Payment Receipt · قبض</span>
      </div>
    </div>
    <div style="text-align:right;flex-shrink:0">
  ...
  <div class="amt-band">
    ...
    <div style="display:flex;align-items:center;justify-content:center;gap:8px;margin-top:6px">
      <span style="font-size:10px;color:#93c5fd;font-weight:600">${methodIcon} ${mLabel(d.method)}</span>
      ${d.daysLate !== undefined && d.periodDueDate ? lateStatusHtml : ''}
    </div>
  </div>
  ...
  <div class="period">
    ...
    <div style="text-align:center">
      ...
      <div style="font-size:18px;font-weight:900;color:#1d4ed8;line-height:1">${d.periodNum}</div>
    </div>` : ''}
  ...
  <div class="ig">
    <div class="ic">
      <span class="il">Product · مصنوعہ</span>
      <span class="iv" style="white-space:normal;line-height:1.3">${d.productName}</span>
      ${d.invoiceNumber ? `<span class="is" style="font-family:monospace">${d.invoiceNumber}</span>` : ''}
    </div>
    <div class="ic">
    ...
    ${d.monthly ? `
    <div class="ic">
      <span class="il">${isDaily ? 'Daily' : 'Monthly'} Qist</span>
